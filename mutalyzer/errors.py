...
def intronic(positions, suggestions=None, assemblies=None):
    details_parts = []
    for ref_id, location_descs in positions.items():
        if len(location_descs) > 1:
            locations = "[" + ";".join(location_descs) + "]"
            details_parts.append(
                f"Intronic positions {locations} used with a non intronic reference sequence {ref_id}."
            )
        else:
            details_parts.append(
                f"Intronic position {location_descs[0]} used with a non intronic reference sequence {ref_id}."
            )
