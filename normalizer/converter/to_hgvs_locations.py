...
def location_to_hgvs(location, crossmap_function, point_function):
    ...
    if location['type'] == 'range':
        new_location['start'] = point_function(crossmap_function(
            location['start']['position']))
        new_location['end'] = point_function(crossmap_function(
            location['end']['position']))
    elif location['type'] == 'point':
        new_location = point_function(crossmap_function(
            location['position']))
    return new_location
...
def crossmap_coordinate_to_genomic_setup():
    crossmap = Crossmap()
    return {'crossmap_function': crossmap.coordinate_to_genomic,
...
def coordinate_to_genomic(variants):
    ...
    for variant in variants:
        ...
        hgvs_indexing_location = location_to_hgvs_ins_fix(variant['location'],
                                                          variant['type'])
        new_variant['location'] = location_to_hgvs(
            hgvs_indexing_location, **crossmap)
...
def coordinate_to_coding(variants, selector_model):
    crossmap = Crossmap(selector_model['exon'], selector_model['cds'][0],
                        inverted=selector_model['inverted'])
    coordinate_variants = []
    for variant in variants:
        new_variant = copy.deepcopy(variant)
        variant['location'] = location_to_hgvs_ins_fix(variant['location'],
                                                       variant['type'])
        if variant['location']['type'] == 'point':
            ...
            coding = fix_crossmap_coding(
                selector_model['exon'][0][0], coding, crossmap)
            coding = fix_crossmap_coding(
...
def to_hgvs_locations(variants, reference_model, selector_id=None):
    ...
    if selector_id is None and get_mol_type(reference_model):
        ...
        selector_model = get_selector_model(reference_model['model'],
                                            selector_id)
    ...
    if coordinate_system == 'g':
        ...
        selector_model['cds'] = [(selector_model['exon'][0][0],
    ...
    return {"reference": reference,
            "variants": hgvs_variants, "coordinate_system": coordinate_system}
