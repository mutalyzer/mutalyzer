...
def check_location_sequence_boundary(hgvs_location, internal_location, reference):
    ...
    if hgvs_location["type"] == "point" and not hgvs_location.get("uncertain"):
        if (
            internal_location["position"] > reference.get_length()
        ...
        if k in ["variants", "deleted", "inserted"]:
            if isinstance(hgvs_model[k], list):
                for i, v in enumerate(hgvs_model[k]):
                    check_points(hgvs_model[k][i], internal_model[k][i], reference)
            elif isinstance(hgvs_model[k], dict):
                check_points(hgvs_model[k], internal_model[k], reference)
        elif k == "location":
            check_location_sequence_boundary(
...
def is_start_end_range_ok(hgvs_location, internal_location):
    if internal_location["start"].get("uncertain") or internal_location["end"].get(
        "uncertain"
    ):
    ...
    if internal_location["start"]["position"] > internal_location["end"]["position"]:
        ...
        add_msg(
            hgvs_location,
            "errors",
            {"code": "ERANGE", "details": "Start location greater than end location."},
        )
        return False
    return True
...
def check_start_end(hgvs_location, internal_location):
    ...
    if (
        internal_location["start"]["type"]
        == internal_location["end"]["type"]
        == "point"
    ):
        is_start_end_range_ok(hgvs_location, internal_location)
    elif (
        internal_location["start"]["type"] == "range"
        and internal_location["end"]["type"] == "point"
    ):
        if not is_start_end_range_ok(
            hgvs_location["start"], internal_location["start"]
        ):
            return
...
def check_locations(hgvs_model, internal_model, references=None):
    ...
    for location_to_check, location_to_report in yield_all_locations_models(
        hgvs_model, internal_model
    ):
        print("--")
